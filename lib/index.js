...
class GoodInflux extends Stream.Writable {
    constructor(host, config) {
        ...
        settings.headers.Authorization = 'Basic ' + Buffer.from(settings.username + ':' + settings.password).toString('base64');
    ...
    _sendMessages(lines, callback) {
      Wreck.request('POST', this._settings.url, { headers: this._settings.headers, payload: lines }).then(() => {
        callback();
      });
    }
}
